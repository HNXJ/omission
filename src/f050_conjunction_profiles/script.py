...
def run_f050():
    ...
    print(f"[debug] Pivoted head:\n{pivoted[['id', 'axab_p1', 'axab_p3', 'axab_ratio']].head(10)}")
    ...
    print(f"[debug] Testing {len(target_units)} units for hardening.")
    ...
    for i, uid in enumerate(target_units):
        ...
        for cond in ['AXAB', 'BXBA', 'RXRR']:
            ...
            if i == 0 and cond == 'AXAB':
                print(f"[debug] Sample Unit {uid} AXAB: p1={p1_trials}, p3={p3_trials}")
    ...
    print(f"[debug] P-value summary: AXAB mean={pivoted['p_axab'].mean():.3f}, BXBA mean={pivoted['p_bxba'].mean():.3f}")
    print(f"[debug] Units with p_axab < 0.1: {(pivoted['p_axab'] < 0.1).sum()}")
    print(f"[debug] Units with p_bxba < 0.1: {(pivoted['p_bxba'] < 0.1).sum()}")
