...
def compute_granger_f(target, source, max_lag=5):
    _, var_res = estimate_ar_coeffs(target, max_lag)
    n = len(target)
    Y = target[max_lag:]
    X = []
    ...
    return max(0, (var_res_model - var_unres) / (var_unres + 1e-12))
...
def run_spiking_granger():
    ...
    plotter = OmissionPlotter(title="Figure f014: Spiking Granger", template="plotly_dark")
    import plotly.graph_objects as go
    plotter.add_trace(go.Scatter(x=times, y=gc_1to2, name="V1 -> PFC", line=dict(color="#CFB87C")), "FF")
    plotter.add_trace(go.Scatter(x=times, y=gc_2to1, name="PFC -> V1", line=dict(color="#9400D3")), "FB")
