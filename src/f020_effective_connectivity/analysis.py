...
def analyze_effective_connectivity(loader: DataLoader, sessions: list, area_pairs: list, condition="AXAB"):
    ...
    for ses in sessions:
        ...
        for a1, a2 in area_pairs:
            ...
            if not lfp1 or not lfp2: 
                log.warning(f"Data missing for {a1} or {a2} in session {ses}")
                continue
            
            # Omission window: 1031 to 1562
            # Average across channels for area-level connectivity
            # loader.get_signal returns a list of arrays. Use the first one (filtered by session).
            s1_trial_avg = np.mean(lfp1[0][:, :, 1031:1562], axis=(0, 1))
            s2_trial_avg = np.mean(lfp2[0][:, :, 1031:1562], axis=(0, 1))
            
            # nitime Granger Analysis
            data = np.vstack([s1_trial_avg, s2_trial_avg])
