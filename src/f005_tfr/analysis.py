    ...
    if "AXAB" in results and "AAAB" in results:
        for area in areas:
            if area in results["AXAB"] and area in results["AAAB"]:
                ...
                # Average across channels and time window per trial
                pow_axab_trials = np.mean(g_axab[:, :, win_mask], axis=(1, 2))
                pow_aaab_trials = np.mean(g_aaab[:, :, win_mask], axis=(1, 2))
                
                # High-Fidelity Permutation Test (n=1000)
                p_val, _ = run_permutation_test(pow_axab_trials, pow_aaab_trials, n_permutations=1000)
                tier, k, stars = get_significance_tier(p_val)
                
                # Store in results metadata
                if "stats" not in results: results["stats"] = {}
                results["stats"][area] = {
                    "p": p_val, "tier": tier, "stars": stars, "test": "Permutation Test (Gamma)"
                }
