...
WIN_SIZE = 100 # Increased from 50ms for stability (Churchland 2010 refinement)
STEP = 5 # Decreased from 10ms for smoother temporal resolution
...
def compute_mmff():
    ...
    # Structure: stats[area][cond][time_bin] = {'means': [], 'vars': []}
                ...
                for u_idx in range(data.shape[1]):
                    ...
                    if area:
                        ...
                        for t in time_bins:
                            ...
                            if mu > 0: # Fano factor needs non-zero mean
                                stats[area][cond][t]['m'].append(mu)
                                stats[area][cond][t]['v'].append(var)

    # --- Mean-Matching Algorithm (Churchland 2010) ---
    ...
    for area in AREA_ORDER:
        for cond in stats[area]:
            # 1. Find the common distribution of means across all time bins for this area/cond
            all_means = []
            for t in time_bins:
            ...
            for t in time_bins:
                ...
                # Subsample to match target distribution
            ...
            # --- Post-hoc Gaussian Smoothing (Churchland 2010 refinement) ---
            trace = np.array(trace)
            nan_mask = np.isnan(trace)
            if not np.all(nan_mask):
                # Interpolate or temporarily fill NaNs for smoothing
                ...
                trace = gaussian_filter1d(trace, sigma=2.0)
                trace[nan_mask] = np.nan # Restore NaNs
    ...
    print("Smoothed MMFF traces saved to checkpoints/area_mmff_traces.json")
