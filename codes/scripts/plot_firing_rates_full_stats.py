...
def plot_final_firing_rates():
    nwb_files = glob.glob('data/sub-*_ses-*_rec.nwb')
    # Accumulate trials per area and condition
    # Structure: area_data[area][cond] = list of trials (averaged within unit)
        ...
        for cond in CONDITIONS:
            ...
            for f in spk_files:
                ...
                for u_idx in range(data.shape[1]):
                    ...
                    if area:
                        # Mean spikes across trials for this unit
                        unit_mean = np.mean(data[:, u_idx, :], axis=0) * 1000
                        smoothed = np.convolve(unit_mean, kernel, mode='same')
                        area_data[area][cond].append(smoothed)
