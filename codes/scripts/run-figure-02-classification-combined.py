...
def load_behavioral(sid, cond):
    fpath = os.path.join(DATA_DIR, f'ses{sid}-behavioral-{cond}.npy')
    if not os.path.exists(fpath): return None
    data = np.load(fpath, mmap_mode='r')
    return data[:, 0:3, :] # Pupil, Eye X, Eye Y

def run_combined_omission_detection():
    all_results = []
    ...
    for sid in SESSIONS:
        ...
        for group_name, (conds, omit_win, delay_win) in OMIT_GROUPS.items():
            ...
            for cond in conds:
                beh = load_behavioral(sid, cond)
                if beh is not None:
                    # Feature: Mean across window for 3 channels
                    omit_list.append(np.mean(beh[:, :, omit_win[0]:omit_win[1]], axis=2))
                    delay_list.append(np.mean(beh[:, :, delay_win[0]:delay_win[1]], axis=2))
            
            if omit_list:
                X_omit = np.vstack(omit_list)
                X_delay = np.vstack(delay_list)
                ...
                if n_min > 5:
                    ...
                    for mode in ['Pupil Only', 'Combined (Pupil+Eye)']:
                        X_feat = X_bal[:, 0].reshape(-1, 1) if mode == 'Pupil Only' else X_bal
                        skf = StratifiedKFold(n_splits=5, shuffle=True, random_state=42)
                        clf = SVC(kernel='linear')
                        scores = cross_val_score(clf, X_feat, y_bal, cv=skf, scoring='accuracy')
    ...
    summary = df.groupby(['group', 'mode'])['accuracy'].agg(['mean', 'std']).reset_index()
    print("\nCombined Behavioral Omission Detection (vs preceding delay):")
    print(summary)
    ...
    fig.update_layout(title="Figure 02: Omission Detection (Pupil vs Combined Behavioral) Within-Trial",
                      yaxis_title="Accuracy (50/50 Balanced)", barmode='group', template="plotly_white")
    
    fig.write_html(os.path.join(OUTPUT_DIR, "FIG_02_Omission_Behavioral_Decoding.html"))
    print("Saved FIG_02.")

if __name__ == '__main__':
    run_combined_omission_detection()
