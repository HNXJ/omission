...
def plot_unit_firing_rate_v6(session, probe_id, local_idx, area_label, filename_tag):
    ...
    fig.update_layout(
        title=dict(
            text=f"Figure 4: Single Unit Dynamics - {filename_tag}<br>Unit: {area_label} | Session: {session} | Local Idx: {local_idx} | Quality: PR>=0.99, SNR>1, RRRR>1Hz",
            font=dict(size=18, family="Arial")
        ),
        xaxis6_title="Time from p1 onset (ms)",
        template="plotly_white", width=1200, height=1500, showlegend=True,
        legend=dict(
            orientation="h",
            yanchor="top",
            y=-0.04,
    ...
    # Save PNG and HTML
    out_base = OUTPUT_DIR / f"figure_4_v6_{filename_tag}"
    fig.write_image(f"{out_base}.png")
    fig.write_html(f"{out_base}.html")
    print(f"Saved: {out_base}.png and .html")

if __name__ == "__main__":
    # Same candidates as v5
    tasks = [
        (230714, 0, 0, "V1_V2", "S_plus"),
        (230629, 1, 1, "V3d_V3a", "S_minus"),
        (230630, 1, 3, "V4_MT", "O_plus")
    ]
    
    for ses, prb, l_idx, area, tag in tasks:
        plot_unit_firing_rate_v6(ses, prb, l_idx, area, tag)
