...
def compute_waveform_metrics(waveform: np.ndarray, fs: float = 30000.0, verbose: bool = VERBOSE) -> dict:
    """Computes waveform duration (trough-to-peak) and half-width."""
    def log(msg):
        if verbose: print(f"""[action] {msg}""")
    
    log(f"""Starting waveform metric computation""")
    
    trough_idx = np.argmin(waveform)
    log(f"""Found trough index: {trough_idx}""")
    
    after_trough = waveform[trough_idx:]
    peak_idx = trough_idx + np.argmax(after_trough)
    log(f"""Found peak index: {peak_idx}""")
    
    duration_samples = peak_idx - trough_idx
    duration_ms = (duration_samples / fs) * 1000.0
    log(f"""Calculated duration: {duration_ms} ms""")
    
    trough_val = waveform[trough_idx]
    half_max = trough_val / 2.0
    log(f"""Calculated half-max: {half_max}""")
    
    left_side = waveform[:trough_idx]
    left_cross = np.where(left_side >= half_max)[0]
    left_idx = left_cross[-1] if len(left_cross) > 0 else 0
    log(f"""Found left crossing index: {left_idx}""")
    ...
    log(f"""Found right crossing index: {right_idx}""")
    
    hw_samples = right_idx - left_idx
    half_width_ms = (hw_samples / fs) * 1000.0
    log(f"""Calculated half-width: {half_width_ms} ms""")
    
    return {"duration": duration_ms, "half_width": half_width_ms}

def classify_neurons(nwb_path: Path, out_dir: Path, verbose: bool = VERBOSE):
    ...
    with get_nwb_io(nwb_path) as (io, nwb):
        ...
        for unit_id, unit_data in units.iterrows():
            ...
            if verbose: print(f"""[action] Processing unit {unit_id} (PR: {presence_ratio}, FR: {firing_rate})""")
            
            waveform = unit_data.get('waveform_mean', np.zeros(100))
            metrics = compute_waveform_metrics(waveform, verbose=verbose)
        ...
        if verbose: print(f"""[Saved] {nwb_path.stem} metrics""")
