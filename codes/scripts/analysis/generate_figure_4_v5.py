...
def plot_unit_firing_rate_v5(session, probe_id, local_idx, area_label, filename_tag):
    ...
    # Save PNG and HTML
    out_base = OUTPUT_DIR / f"figure_4_v5_{filename_tag}"
    fig.write_image(f"{out_base}.png")
    fig.write_html(f"{out_base}.html")
    print(f"Saved: {out_base}.png and .html")

if __name__ == "__main__":
    # Candidates from find_specific_neurons_v3.py (PR >= 0.99, SNR > 1.0, RRRR > 1.0Hz)
    tasks = [
    ...
    for ses, prb, l_idx, area, tag in tasks:
        plot_unit_firing_rate_v5(ses, prb, l_idx, area, tag)
