...
OUTPUT_DIR = Path(r'D:\drive\omission\outputs\oglo-figures\figure-5')
...
def generate_triplets():
    ...
    # Compute PCA trajectories per area
    area_pca = {}
    for area in CANONICAL_AREAS:
        ...
        G1_trials = np.stack([x[:min_t, :] for x in G1_neurons], axis=-1)
        G2_trials = np.stack([x[:min_t, :] for x in G2_neurons], axis=-1)
        
        G1_mean = G1_trials.mean(axis=0)
    ...
    # Define groups
    group1 = ['V1', 'V2', 'V3d']
    group2 = ['V3a', 'V4', 'MT', 'MST']
    group3 = ['TEO', 'FST', 'FEF', 'PFC']
    
    triplets = list(itertools.product(group1, group2, group3))
    print(f"Total triplets to generate: {len(triplets)}")

    for t_idx, (a1, a2, a3) in enumerate(triplets):
        fig = go.Figure()

        for area in [a1, a2, a3]:
            if area not in area_pca: continue
            
            G1_mean_pca = area_pca[area]['G1']
            G2_mean_pca = area_pca[area]['G2']
        ...
        fig.update_layout(
            title=f'Figure 5: Population State-Space Dynamics ({a1} - {a2} - {a3})<br><i>Color: Brain Area | Solid: Standard Sequence | Dashed: Omission</i>',
            scene=dict(
        ...
        out_html = OUTPUT_DIR / f'figure_5_state_space_{a1}_{a2}_{a3}.html'
        fig.write_html(out_html)
        if (t_idx + 1) % 10 == 0 or t_idx == len(triplets) - 1:
            print(f"Generated {t_idx + 1} plots...")
