...
# Paths
BASE_DIR = Path('D:/Analysis/Omission/local-workspace')
UNITS_LAYERED_PATH = BASE_DIR / 'checkpoints' / 'real_omission_units_layered_v3.csv'
LATENCY_PATH = BASE_DIR / 'checkpoints' / 'omission_latencies_v2.csv'
VFLIP_SUMMARY_PATH = BASE_DIR / 'LFP_Extractions' / 'vflip2_summary.txt'
NWB_DIR = BASE_DIR / 'data'
OUTPUT_PATH = BASE_DIR / 'checkpoints' / 'real_omission_units_layered_v3_final.csv'

def parse_vflip_summary(path):
    mappings = {}
    if not path.exists(): 
        print("vFLIP summary not found at {}. Layer mapping will be incomplete.".format(str(path)))
        return mappings
...
def map_layers():
    if not UNITS_LAYERED_PATH.exists():
        print("Error: Units layered file not found at {}".format(str(UNITS_LAYERED_PATH)))
        return
