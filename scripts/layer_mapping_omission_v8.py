"""
layer_mapping_omission_v8.py: Maps identified omission neurons to cortical layers (Superficial, L4, Deep)
...
# Paths
UNITS_LAYERED_PATH = os.path.join('D:', 'Analysis', 'Omission', 'local-workspace', 'checkpoints', 'real_omission_units_layered_v3.csv')
LATENCY_PATH = os.path.join('D:', 'Analysis', 'Omission', 'local-workspace', 'checkpoints', 'omission_latencies_v2.csv')
VFLIP_SUMMARY_PATH = os.path.join('D:', 'Analysis', 'Omission', 'local-workspace', 'LFP_Extractions', 'vflip2_summary.txt')
NWB_DIR = os.path.join('D:', 'Analysis', 'Omission', 'local-workspace', 'data')
OUTPUT_PATH = os.path.join('D:', 'Analysis', 'Omission', 'local-workspace', 'checkpoints', 'real_omission_units_layered_v3_final.csv')

def parse_vflip_summary(path):
    mappings = {}
    if not os.path.exists(path): 
        print("vFLIP summary not found at {}. Layer mapping will be incomplete.".format(path))
        return mappings
...
def map_layers():
    ...
    print("Loaded {} crossover mappings.".format(len(vflip_map)))
    ...
    for sid in df_omit['session_id'].unique():
        sid_str = str(int(sid))
        print("  Processing Session {}".format(sid_str))
        
        nwb_files = [f for f in os.listdir(NWB_DIR) if sid_str in f and f.endswith('.nwb')]
        if not nwb_files: 
            print("    No NWB file found for session {}. Skipping.".format(sid_str))
            continue
        ...
        try:
            ...
            print("    Error processing {}: {}".format(sid_str, e))
