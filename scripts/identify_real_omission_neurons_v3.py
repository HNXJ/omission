"""
identify_real_omission_neurons_v3.py: Detects "Real" omission neurons using corrected 1000ms offset.
...
def detect(session_id, probe_id):
    ...
    for f_path in omit_files:
        ...
        elif 'AAXB' in cond or 'BBXA' in cond or 'RRXR' in cond:
            omit_win = P3_WIN
        ...
        for u in range(n_units):
            # Calculate rates (Hz)
            def get_rate(win):
                return np.mean(np.sum(spikes[:, u, win[0]:win[1]], axis=1)) / (win[1]-win[0]) * 1000
...
def main():
    ...
    if not df.empty:
        ...
        df.to_csv(os.path.join(OUTPUT_DIR, 'real_omission_units_v3.csv'), index=False)
